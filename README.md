...
本地优先的科研/工作流程管理桌面应用（Tauri 2）。发布目标：**Windows x64 + macOS 12+（universal：Apple Silicon + Intel，DMG）**——macOS 适配代码已入库，DMG 需在 macOS 机构建（流程见 [docs/release.md](docs/release.md) §3）。把手写「思路整理」工作流数字化：**空间 → 项目 → 任务**三级层级 + 双层上下文（项目整体上下文 / 任务上下文）+ 周文档。

第二步（已规划、未开始）：**人只说自然语言，agent 维护整个项目地图**——口述新任务/新项目/任务转折即可，agent 经 MCP 工具接口执行并定时产出日报/周报。

## 状态

**M3 完成 + 种子发布加固**（2026-09-11，M1–M3 全部落地并实机验证）：数据落盘（Markdown 文件树）、活动日志、文件监听、收件箱、今日视角、终端面板、数据目录初始化、周文档一键生成、全文搜索、项目归档、markdown 预览之上，按发布前审阅补齐数据安全九项：事务式可恢复写入、单实例、外部编辑冲突防护、每日快照健康（git 缺失界面可见）、未保存草稿退出保护、周文档覆盖确认、周报正确性修正、watcher 路径归一化、种子手册（隐私边界 + 脱敏反馈模板）。**macOS 实机验证完成**（2026-09-12，PR #3）：universal DMG（x86_64+arm64）、Finder 启动 PATH、应用内终端三件套、冲突/快照/.trash/单实例全矩阵通过；修复 macOS 数据层初始化 bug（fs 插件点文件作用域）；确认 macOS 15+ 无「右键打开」Gatekeeper 通道，安装指引已按系统版本分流。下一步：干净 Windows 环境实测 → release tag → 受控种子版；随后第二步阶梯 2a。计划见 [PLAN.md](PLAN.md)。
...
- Tauri 2 桌面壳（plugin-fs / plugin-shell / plugin-opener / single-instance / global-shortcut；生产线启用 CSP，见 [docs/release.md](docs/release.md) §5）
